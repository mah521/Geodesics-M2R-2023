...
def plot_plane_and_torus(f, normal_f, point_2d_list, epsilon):
    ...
    line_3d = [np.array(f(point[0], point[1])) + epsilon * np.array(normal_f(point[0], point[1])) for point in point_2d_list]
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=x_line, y=y_line, mode='lines', line=dict(width=4)))
    fig1.update_layout(scene=dict(xaxis_title='', yaxis_title='', xaxis_showgrid=False, yaxis_showgrid=False))
    # Uncomment to plot in R^2
    # fig1.show()
 
        # Plot the line in R^3 and the torus
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter3d(x=x_line_3d, y=y_line_3d, z=z_line_3d, mode='lines', line=dict(width=8, color='red')))
    fig2.add_trace(go.Surface(x=x_torus, y=y_torus, z=z_torus, opacity=1, showscale=False))
...
def gradient_L(f, jacobian_f, point_2d_list):
    ...
    for i in range(len(point_2d_list)):
        if i == 0:  # First point
            new_val = np.add(H(f, jacobian_f, point_2d_list[i], point_2d_list[i+1]), H(f, jacobian_f, point_2d_list[n], point_2d_list[n-1]))
        elif i == n:  # Last point
            new_val = np.add(H(f, jacobian_f, point_2d_list[0], point_2d_list[1]), H(f, jacobian_f, point_2d_list[n], point_2d_list[n-1]))
        else:  # Other points
            new_val = np.add(H(f, jacobian_f, point_2d_list[i], point_2d_list[i-1]), H(f, jacobian_f, point_2d_list[i], point_2d_list[i+1]))
...
def gradient_descent(f, jacobian_f, N, alpha, point_2d_list, threshold):
    ...
    for i in range(N):
        ...
        # Set all values in the gradient that are below the threshold * max_gradient to zero
...
def animate_gradient_descent(f, normal_f, steps, step_size, max_array):
    ...
    for i, step in enumerate(steps):
        line_3d = [np.array(f(point[0], point[1])) + epsilon * np.array(normal_f(point[0], point[1])) for point in step]
        x_line_3d = [point[0] for point in line_3d]
        y_line_3d = [point[1] for point in line_3d]
        z_line_3d = [point[2] for point in line_3d]
        
        frames.append(go.Frame(data=[go.Scatter3d(x=x_line_3d, y=y_line_3d, z=z_line_3d, mode='lines', line=dict(width=8, color='red')),
                                     go.Surface(x=x_torus, y=y_torus, z=z_torus, opacity=1, showscale=False),
                                     ],
                                     layout=go.Layout(title_text="Max gradient at step {}: {}".format(i*step_size, max_array[i]))
                                     ))
    ...
    # Plot the line in R^3 and the torus
    fig = go.Figure(
        data=[go.Scatter3d(x=x_line_3d, y=y_line_3d, z=z_line_3d, mode='lines', line=dict(width=8, color='red')),
              go.Surface(x=x_torus, y=y_torus, z=z_torus, opacity=1, showscale=False)],
        layout=go.Layout(
    ...
    fig.update_layout(scene=dict(xaxis_title='', yaxis_title='', zaxis_title='',xaxis_showgrid=False, yaxis_showgrid=False, zaxis_showgrid=False,
    aspectmode='data',
...
# Compute the normalized cross product of the gradients of f with respect to x and y
normal_f = lambda u, v: np.cross(jacobian_f(u, v)[:, 0], jacobian_f(u, v)[:, 1]) / np.linalg.norm(np.cross(jacobian_f(u, v)[:, 0], jacobian_f(u, v)[:, 1]))
...
point_2d_list = generate_perturbed_line(n, m, N, lambda_val, 1) # Generate the curve
...
max_array = []
steps, max_array = gradient_descent(f, jacobian_f, K, alpha, point_2d_list, threshold)
animate_gradient_descent(f, normal_f, steps,step_size, max_array)
